...
class HashDB:
    ...
    def __init__(self, target_dir: pathlib.Path) -> None:
        ...
        self.db_path = target_dir / DB_FILENAME
    ...
    def insert(
        ...
        *,
        ...
        self._conn.execute(
            "INSERT INTO files (hash, file_size, file_path, date_taken, import_date, source_path) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (hash, file_size, file_path, date_taken, import_date, source_path),
        )
        self._conn.commit()

    def hash_exists(self, hash: str) -> bool:
        """Check if a hash exists in the DB."""
        cursor = self._conn.execute("SELECT 1 FROM files WHERE hash = ?", (hash,))
        return cursor.fetchone() is not None

    def get_by_hash(self, hash: str) -> list[dict[str, object]]:
        """Get all records for a given hash."""
        cursor = self._conn.execute("SELECT * FROM files WHERE hash = ?", (hash,))
    ...
    def delete_by_hash_and_path(self, hash: str, file_path: str) -> None:
        ...
        self._conn.execute(
            "DELETE FROM files WHERE hash = ? AND file_path = ?",
            (hash, file_path),
        )
        self._conn.commit()
    ...
    def source_path_imported(self, path: str) -> bool:
        ...
        cursor = self._conn.execute(
            "SELECT 1 FROM imports WHERE source_path = ?", (path,)
        )
        return cursor.fetchone() is not None
    ...
    def get_import(self, source_path: str) -> dict | None:
        ...
        cursor = self._conn.execute(
            "SELECT * FROM imports WHERE source_path = ?", (source_path,)
        )
        row = cursor.fetchone()
    ...
    def record_import(
        ...
        self._conn.execute(
            "INSERT OR IGNORE INTO imports (source_path, hash, file_path) VALUES (?, ?, ?)",
            (source_path, hash, file_path),
        )
        self._conn.commit()
